...
- **No options. No spot single-name equities. No unleveraged broad-market
  ...
  ETFs** — NVDL/NVD, TSLL/TSLZ, MSTU/MSTZ, CONL, and from 2026-07-02
  PLTU/PLTD, AMZU/AMZD, GGLL/GGLS, METU/METD — are allowed; they are
...
- Cycle cadence: **every 4 hours during market hours, weekdays only**. The market_gate stage queries Alpaca's clock and short-circuits weekends/holidays/after-hours without LLM cost. Within market hours, the orchestrator-meta agent (Sonnet, effort medium — lowered from high 2026-08-31, user-authorized: a bounded scheduling pick with a deterministic fallback doesn't repay high-effort thinking tokens) picks the actual next-run timestamp (bounded 1–24h). A meta "review" pick scheduled more than 24h before the next market open (e.g. a weekend) is deterministically downgraded to "trade" so the gate $0-skips it instead of billing ~$0.13 for reflection nothing can act on (2026-08-31, user-authorized).
- Crash resilience (2026-08-31): an unhandled pipeline exception writes `run_dir/error.json` + a `stage="pipeline"` decision row (`status="error"`; cost-cap stops log `"aborted"` — a designed guardrail, not a gate failure) and a conservative fresh `next_run.json` (30min retry; per-run cap +4h; daily cap next UTC day; never when halted), so a crash can no longer stall the scheduler until the daily 13:30 UTC fallback timer. The §Promotion failure gate counts `error` rows PLUS crashed run dirs that recorded nothing.
...
- Anthropic prompt caching on every static stage system prompt — kept enabled, with eyes open about its measured limits (2026-08-31 audit over ~900 calls, user-authorized spec correction): the four stage prompts share no prefix and cycles run 1–24h apart against the 5-minute cache TTL, so hits only occur on same-stage schema retries and critic-triggered constructor reruns (~0–5% observed; roughly cost-neutral overall — the 1.25× write premium on ~3k-token prompts is offset by those rerun reads). Do NOT expect cross-stage or cross-cycle cache savings at this cadence, and do NOT restructure the prompts to chase them (role-specific system prompts and byte-stability across paper→live matter more). Cache metrics are computed input-side from `state/costs.jsonl`; decision rows carry a real per-stage aggregate.
...
- Sharpe ≥ 0.5 on paper after modelled costs (sanity floor, not a target). Measured on the dense SPY-trading-day-calendar, $2,500-anchored series (quiet days count as 0% returns) — the scorecard's primary rows; the EOD-sampled nav_history variant inflates Sharpe by collapsing calendar gaps and is shown as labeled secondary rows only (2026-08-31 methodology fix).
